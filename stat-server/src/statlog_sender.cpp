...
{
	//这里处理精灵大作战的自定义数据，不入库
    if(mmap_file.get_file_name().find("630_game_custom") != string::npos) {
        return SLP_OK;
    }

    if (get_sendtodb_auto() && get_outbox_filecount() >= get_rename_trigger())    // auto mode on and rename_triggered
        return SLP_OK;
    ...
    while(!::g_stop && !mmap_file.eof())
    {
        ...
        if(ret < 0)
        {
            ...
            ERROR_LOG("send line failed, %lu line: %s", len, (char*)line);
            ret = SLP_ERR_BREAK;
            goto DONE;
        }
