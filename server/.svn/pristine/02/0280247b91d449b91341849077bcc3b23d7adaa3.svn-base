...
do
	...
	${HADOOP_PATH}hadoop jar ${HADOOP_JAR_PATH} \
		...
		-output ${DAY_DIR}/$date/new-keep-${n}-ahero
	...
	${HADOOP_PATH}hadoop jar ${HADOOP_JAR_PATH} \
		...
		-input ${DAY_DIR}/$date/new-keep-${n}-ahero/part-* \
		-addMos "percent,org.apache.hadoop.mapred.TextOutputFormat,org.apache.hadoop.io.Text,org.apache.hadoop.io.Text" \
		-output ${SUM_DIR}/$date/new-keep-${n}-ahero

	$DB_UPLOAD -type 2 -date $nday -task 19 -path ${SUM_DIR}/$date/new-keep-${n}-ahero/part*
	$DB_UPLOAD -type 2 -date $nday -task 20 -path ${SUM_DIR}/$date/new-keep-${n}-ahero/percent*
	exit
done
