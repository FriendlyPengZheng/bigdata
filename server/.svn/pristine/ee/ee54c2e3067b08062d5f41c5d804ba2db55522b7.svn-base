 ...
 * @date 2013-11-21
 */
public class PayAmtCountUcountDistReducer extends MRBase implements
	...
	public void reduce(Text key, Iterator<Text> values,
		...
		//付费金额
		if(this.amount != null){
			outputValue.set(String.format("%.1f", amt));
		...
		//付费人数
		if(this.ucount != null){
			outputValue.set(String.format("%d", ucount));
		...
		//付费arppu
		if(this.arppu != null){
			outputValue.set(String.format("%.4f", amt/ucount));
		...
		if(this.amount_distr != null){
			...
			while(it.hasNext()){
                ...
                dist = entry.getKey();
				v = entry.getValue();
				outputValue.set(dist.toString(0.01) + "\t" + v.get());
		...
		if(this.count_distr != null){
			...
			while(it.hasNext()){
                ...
                dist = entry.getKey();
				v = entry.getValue();
				outputValue.set(dist.toString() + "\t" + v.get());
