...
int send_rqst(int fd, const char *p_rqst_msg, char *p_resp_buf, uint32_t buf_len, int timeout)
{
    if(p_rqst_msg == NULL)
        return 0;

	int rv = -1;
	uint32_t rqst_msg_rmng_len = *(uint32_t *)p_rqst_msg;
	uint32_t resp_buf_used_len = 0;
    int ret = 0;
	...
	for (;;) 
    {
        ...
        {
			ERROR_LOG("ERROR: resp_buf_used_len: %d >= buf_len: %d", resp_buf_used_len, buf_len);
			return -1;
		}
        ...
        {
			ERROR_LOG("ERROR: select timeout %d times.", timeout_count);
        ...
        else
        {
            if (FD_ISSET(fd, &write_set))
            {
                if (rqst_msg_rmng_len > 0)
                {
                    rv = write(fd, p_rqst_msg, rqst_msg_rmng_len);
            ...
            {
                rv = read(fd, p_resp_buf + resp_buf_used_len, buf_len - resp_buf_used_len);
                if (rv == -1 && errno != EAGAIN ) 
                {
                ...
                else
                {
                    ...
                    if (resp_buf_used_len >= sizeof(uint32_t) && 
                            resp_buf_used_len >= *(uint32_t *)p_resp_buf)
                    {
    ...
    // write successfully but error/timeout on wait for read response.
    if(ret == -1 && rqst_msg_rmng_len == 0)
    {
        ret = -2;
    }
