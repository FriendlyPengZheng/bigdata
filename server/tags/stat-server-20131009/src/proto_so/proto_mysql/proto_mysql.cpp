...
int proto_process(const ss_message_header_t &ss_msg_hdr, void *p_data)
{
    ...
    int ret = 0;
    ret = send_rqst(g_srv_vec[srv_idx].fd, (const char *)&req, (char *)&res, sizeof(res), g_send_rqst_timeout);

    // 失败时关闭连接，失败有可能是读写socket或select超时造成的。
    // 当write socket成功但是在read socket失败，认为是数据处理成功了，否则统计客户端会一直重发，导致stat-mysql收到重复的数据。
    if(ret < 0)
    {
