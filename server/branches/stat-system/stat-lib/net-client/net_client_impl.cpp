...
int create_net_client_instance(i_net_client **pp_instance)
{
    ...
    {
        *pp_instance = dynamic_cast<i_net_client*>(p_instance);
        return 0;
    }
}
...
int c_net_client_impl::init(int server_addr, int server_port, int timeout)
{
    ...
    if(connect_to_server(timeout) != 0)
    {
        ...
        ERROR_LOG("can not connect to server(%s: %d) now.", inet_ntoa(tmp_addr), server_port);
        return -1;
    }
...
int c_net_client_impl::send_data_atomic(char *p_data, int data_len)
{
    ...
    {
        ERROR_LOG("It doesn't have enough space");
        return -1;
    }
...
int c_net_client_impl::ping(int timeout)
{
    if(m_sock_fd < 0)
    {
        if(connect_to_server(timeout) != 0)
        {
            ERROR_LOG("cat not connect to server");
            return -1;
        }
    }
...
int c_net_client_impl::connect_to_server(int timeout)
{
    ...
    if(m_sock_fd < 0)
    {
        ERROR_LOG("errno[%d]:%s",errno, strerror(errno));
        m_sock_fd = -1;
        return -1;
    }
