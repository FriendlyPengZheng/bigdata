...
{
	DEBUG_LOG("proto_ucount: proto_init");
	...
	for (int i = 0; ; ++i) {
		...
		snprintf(server_name, sizeof(server_name) - 1, "stat_ucount_%d", i);
		memset(buffer, 0, sizeof(buffer));
		if (p_config->get_config("proto-ucount", server_name, buffer, sizeof(buffer) - 1) != 0) {
			break;
		}

		char *p_semicolon = strchr(buffer, ':');
		if (p_semicolon == NULL || p_semicolon - buffer >= (int)sizeof(g_p_srv_list[g_srv_count - 1].ip)) {
			ERROR_LOG("ERROR: proto-ucount config: %s: %s", server_name, buffer);
			goto ERROR;
		}
		...
		DEBUG_LOG("proto_ucount: stat_ucount_%d: ip: %s: port: %d", g_srv_count - 1, 
...
{
	DEBUG_LOG("proto_ucount: get_proto_id");
	...
	char sql[] = "SELECT id FROM t_message_protocol WHERE name = 'UCOUNT';";
	...
	*proto_count = 0;
	while (row != NULL) {   
		...
		// TODO:
		p_proto_id[*proto_count] = atoi(row[0]) + 100;
		*proto_count += 1;
	...
	DEBUG_LOG("proto_ucount: proto_id: %d", *p_proto_id);
...
int proto_process(const ss_message_header_t &ss_msg_hdr, void *p_data)
{
	...
	if (*(uint32_t *)p_data == 0 || *(uint32_t*)p_data >= 1073741824) {  // 过滤米米号为0和大于10亿的消息
		return 0;
	}
	...
	write_request_msg_t *p_write_request_msg = (write_request_msg_t *)g_send_buf;
	p_write_request_msg->msg_len = sizeof(write_request_msg_t) + sizeof(p_write_request_msg->rtu[0]);
	p_write_request_msg->msg_id = WRITE_MSG_ID;
	p_write_request_msg->rtu[0].report_id = ss_msg_hdr.report_id;
	p_write_request_msg->rtu[0].timestamp = ss_msg_hdr.timestamp;
	p_write_request_msg->rtu[0].unumber = *(uint32_t *)p_data; 

	if (send_rqst(g_p_srv_list[srv_idx].fd, (char *)p_write_request_msg, g_recv_buf, sizeof(g_recv_buf), g_send_rqst_timeout) != 0) {
	...
	return p_write_response_msg->result[0];                // 落唯一数的结果 0：成功 －1： 失败
}

int proto_uninit()
{
	DEBUG_LOG("proto_ucount: proto_uninit");
