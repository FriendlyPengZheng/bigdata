...
int main(int argc, char **argv)
{
	...
	p_handle = dlopen("proto_ucount.so", RTLD_LAZY);
	if (p_handle == NULL) {
	...
	*(void **)(&p_proto_uninit) = dlsym(p_handle, "proto_uninit");
	...
	for (int i = 0; i != 10000; ++i) {
		ss_msg_hdr.len = sizeof(ss_msg_hdr);
		ss_msg_hdr.report_id = i;
		ss_msg_hdr.timestamp = time(NULL);
		ss_msg_hdr.cli_addr = 0;
		ss_msg_hdr.proto_id = 5;

		ucount = i;

		if (p_proto_process(ss_msg_hdr, &ucount) != 0) {
			fprintf(stderr, "ERROR: p_proto_process: report_id: %d ucount: %u i: %d\n", ss_msg_hdr.report_id, ucount, i);
			goto ERROR;
		}
	}
