...
global.statTM.CurrencyType = require("./currencyType.js");
global.statTM.PayReason = require("./payReason.js");
global.statTM.TaskType = require("./taskType.js");
global.statTM.UnsubscribeChannel = require("./unsubscribeChannel.js");
global.statTM.NewTransStep = require("./newTransStep.js");


global.statTM.task_stid = new Array(new Array("", "", ""),
...
global.statTM.new_trans_stid = new Array("", "fGetRegSucc", "fLoadRegSucc", "fSendLoginReq",
 ...
 *
 */
StatLogger.prototype.modifyGid=function(game)
{
   this.m_appId = game;
}

StatLogger.prototype.modifyZid=function(zone)
{
   this.m_zoneId = zone;
}

StatLogger.prototype.modifySid=function(svr)
{
   this.m_svrId = svr;
}

StatLogger.prototype.modifyPid=function(site)
{
   this.m_siteId = site;
}

/**
 * 为了方便游戏测试数据,目前可以动态修改创建文件的路径,递归创建文件夹。
 */

StatLogger.prototype.modifyPath=function(path)
{
    if (!statCommon.stat_makeDirSync(path+"/inbox"),0777) {
      this.m_path = path;
     }
}
...
StatLogger.prototype.log=function(stat_name,sub_stat_name,acct_id,player_id,info)
{
    ...
    {
        console.warn("log()方法参数类型不匹配!");
        return;
    }
    statCommon.stat_trim_underscore(stat_name);
    statCommon.stat_trim_underscore(sub_stat_name);

    if(!(this._is_valid_common_utf8_parm(stat_name,1,256)
        && this._is_valid_common_utf8_parm(sub_stat_name,1,256)))return;
...
StatLogger.prototype.new_trans = function (step, acct_id) {
    ...
    {
        console.warn("new_trans()方法参数类型调用不匹配!");
        return;
    }
    if (!this.is_valid_newtransstep(step)) {
        console.warn("不是合法的步骤!");
        return;
    }
...
StatLogger.prototype.get_new_trans_step = function (step) {
    return statTM.new_trans_stid[step];
}
StatLogger.prototype.is_valid_newtransstep = function (step) {
    return (typeof step == "number") && (step > statTM.NewTransStep.NW_BEGIN) && (step < statTM.NewTransStep.NW_END);
}
...
StatLogger.prototype.cancel_acct = function (acct_id, channel) {
    ...
    {
        console.warn("cancel_acct()方法参数类型不匹配!");
        return;
    }
    if (acct_id.length == 0)
        acct_id = "-1";
...
StatLogger.prototype.unsubscribe = function (acct_id, uc) {
    ...
    {
        console.warn("unsubscribe()方法参数类型不匹配！");
        return;
    }
    if (acct_id.length == 0) {
        acct_id = "-1";
...
StatLogger.prototype.abort_task = function (type, acct_id, task_name, lv) {
    ...
    {
        console.warn("abort_task() 方法参数类型不匹配!");
        return;
    }
    this._do_task(type, acct_id, task_name, lv, 2);
}
...
StatLogger.prototype.finish_task = function (type, acct_id, task_name, lv) {
    if(typeof type!="number" || typeof acct_id!="string" || typeof task_name!="string" || typeof lv!="number")
    {
        console.warn("finish_task() 方法参数类型不匹配!");
        return;
    }
    this._do_task(type, acct_id, task_name, lv, 1);
}
/**
 *
 * @param type
 ...
 * @param step
 */
StatLogger.prototype._do_task = function (type, acct_id, task_name, lv, step) {
    statCommon.stat_trim_underscore(task_name);//TODO

    if (!(this._is_valid_tasktype(type)
        && this._is_valid_common_utf8_parm(task_name, 1, 256)))
        return;
    ...
    oss += this.set_basic_info(this._get_task_stid(type, step), task_name, ts, acct_id, "-1");
...
StatLogger.prototype._get_task_stid = function (type, stage) {
    //console.log(task_stid[type][stage]);
    return statTM.task_stid[type][stage];
}
 ...
 * @returns {boolean}
 */
StatLogger.prototype._is_valid_tasktype = function (type) {
    return (typeof type == "number") && (type > statTM.TaskType.TASK_BEGIN ) && (type < statTM.TaskType.TASK_END);
}
/**
...
StatLogger.prototype.accept_task = function (type, acct_id, task_name, lv) {
    if(typeof type!="number" || typeof acct_id!="string" || typeof task_name!="string" || typeof lv!="number")
    {
        console.warn("accept_task() 方法参数类型不匹配!");
        return;
    }
    //do_task就不需要
    this._do_task(type, acct_id, task_name, lv, 0);
}
/**
 *
 * @param acct_id
...
StatLogger.prototype.use_golds = function (acct_id, is_vip, reason, amt, lv) {
    if(typeof acct_id!="string" ||
        ...
        typeof lv!="number")
    {
        console.warn("use_golds()方法调用参数不匹配");
        return;
    }
    ...
    if (!(amt > 0
        && amt <= 100000)
        && this._is_valid_common_utf8_parm(reason, 1, 256)
        && this.is_valid_lv(lv))
        return;
...
StatLogger.prototype.buy_item = function (acct_id, isvip, lv, pay_amount, outcome, outcnt) {
    ...
    {
        console.warn("buy_item()方法调用参数不匹配");
        return;
    }
    if (!this.is_valid_lv(lv)
        && pay_amount > 0
        && this._is_valid_common_utf8_parm(outcome, 1, 256)
        && outcnt > 0)
        return;
    if (acct_id.length == 0) {
        acct_id = "-1";
...
StatLogger.prototype.use_golds_buyitem = function (acct_id, is_vip, amt, lv) {
    ...
    {
        console.warn("use_golds_buyitem()方法调用参数类型不合法!")
        return;
    }
    if (acct_id.length == 0) {
        acct_id = "-1";
...
StatLogger.prototype.obtain_golds = function (acct_id, amt) {
    ...
    {
        console.warn("obtain_golds()方法调用参数类型不匹配!");
        return;
    }
...
/**
 * 
 *  获得精灵接口,String,boolean,int,String
 *
 */
StatLogger.prototype.obtain_spirit = function(acct_id,isvip,lv,spirit){
    
    if(typeof acct_id!="string" || typeof isvip !="boolean" || typeof lv !='number' || typeof spirit !='string')
    {
        console.warn("obtain_spirit()方法调用参数类型不匹配!");
        return;
    }
    if(!this.is_valid_lv(lv))
	return;
...
/**
 * 
 *  失去精灵接口,String,boolean,int,String
 *
 */
StatLogger.prototype.lose_spirit = function(acct_id,isvip,lv,spirit){
...
StatLogger.prototype.level_up = function (acct_id, race, lv) {
    ...
    {
        console.warn("level_up()方法调用参数类型不匹配!");
        return;
    }
    if (!this.is_valid_lv(lv))
        return;
...
StatLogger.prototype.verify_passwd = function (acct_id, cli_ip, ads_id) {
    ...
    {
        console.warn("verify_passwd()方法调用参数类型不匹配!");
        return;
    }
    if (acct_id.length == 0) {
        acct_id = "-1";
...
StatLogger.prototype.online_count = function (cnt, zone) {
    ...
    {
        console.warn("online_count()方法调用参数不匹配!");
        return;
    }
    if (cnt < 0) {
        return;
...
StatLogger.prototype.login_online = function (acct_id, player_id, race, isvip, lv, cli_ip, ads_id, zone) {
    if(typeof acct_id!="string" ||
        ...
        typeof  zone!="string")
    {
        console.warn("login_online()方法调用参数不匹配!");
        return;
    }
    var ts = ~~(Date.now() / 1000) + "";
    //console.log(ts);
    ...
    if(player_id.length == 0)
    {
       lgac_info = this.set_basic_info("_lgac_", "_lgac_", ts, acct_id,
            "-1")     
        + "\t_vip_="
...
StatLogger.prototype.set_device_info = function (ads) {
   /**
    **无论传递进来的是否是字符串首先给其转成字符串，然后判断其字符串的长度是否是0，是0，给其加个默认值，否则返回正确的ads值
    */
    if((ads+"").length==0)
    {
       ads="-1"; 
...
StatLogger.prototype.reg_role = function (acct_id, player_id, race, cli_ip, ads_id) {
    if(typeof acct_id!="string" ||
        ...
        typeof  ads_id!="string")
    {
        console.warn("reg_role()方法调用参数不匹配!");
        return;
    }
    var ts = ~~(Date.now() / 1000) + "";
    var op = "\t_op_=";
...
StatLogger.prototype.pay = function (acct_id, isvip, pay_amount, currency, pay_reason, outcome, outcnt, pay_channel) {
    ...
    {
        console.warn("pay()方法调用参数不匹配!");
        return;
    }
    if (!(typeof pay_amount == "number"
        && pay_amount > 0
        && this._is_valid_currency(currency)
        && this._is_valid_payreason(pay_reason)
        && outcnt > 0
        && (outcome.length > 0 || pay_reason != statTM.PayReason.PAY_BUY)
        && this._is_valid_common_utf8_parm(outcome, 0, 256)
        && this._is_valid_common_utf8_parm(pay_channel, 1, 256))) {
        return;
    }
    //console.log("1111");
    if (acct_id == "") {
    ...
    var reason;
    switch (pay_reason) {
        case statTM.PayReason.PAY_CHARGE:
            reason = "_buycoins_";
            break;
        case statTM.PayReason.PAY_VIP:
            reason = "_vipmonth_";
            break;
        case statTM.PayReason.PAY_BUY:
            reason = "_buyitem_";
            break;
        case statTM.PayReason.PAY_FREE:
            reason = "_costfree_";
            break;
        default:
            reason = "";
    ...
    var oss = "";
    if (pay_reason != statTM.PayReason.PAY_FREE) {
...
StatLogger.prototype.do_buy_item = function (acct_id, isvip, lv, pay_amount, pay_type, outcome, outcnt) {
    ...
    {
        console.warn("do_buy_item()方法调用参数不匹配!");
        return;
    }
    var ts = ~~(Date.now() / 1000) + "";
    var oss = "";
...
StatLogger.prototype.do_buy_vip = function (acct_id, pay_amount, amt) {
    ...
    {
        console.warn("do_buy_vip()方法调用参数不匹配!");
        return;
    }
    var ts = ~~(Date.now() / 1000) + "";
    var oss="";
...
StatLogger.prototype.do_obtain_golds = function (acct_id, reason, amt) {
    ...
    {
        console.warn("do_obtain_golds()方法调用参数不匹配!");
        return;
    }
    var ts = ~~(Date.now() / 1000) + "";
    var oss="";
 ...
 * @returns {boolean}
 */
StatLogger.prototype._is_valid_currency = function (ccy) {
    return (typeof ccy == "number") && (ccy > statTM.CurrencyType.CCY_BEGIN ) && (ccy < statTM.CurrencyType.CCY_END);
}
 ...
 * @returns {boolean}
 */
StatLogger.prototype._is_valid_payreason = function (r) {
    return (typeof r == "number") && (r > statTM.PayReason.PAY_BEGIN) && (r < statTM.PayReason.PAY_END);
}
 ...
 * @returns {boolean}
 */
StatLogger.prototype._is_valid_race = function (race) {
    return (statCommon.size_between(race, 1, 256)) && (!statCommon.key_no_invalid_chars(race));
}

StatLogger.prototype._is_valid_common_utf8_parm = function (parm, min, max) {
