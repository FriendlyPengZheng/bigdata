/**
 * Created by looper on 2016/12/25.
 * TAOMEE sdk公共工具类
 */
var fs = require("fs");
const os = require("os");
...
exports.stat_makeDir = function (path) {
    ...
    fs.exists(path, function (exists) {
        var flag;
        if (exists) {
            console.warn(path + "路径存在,不需要重新重新创建!");
            return true;
            // return flag;
        }
        else {
            fs.mkdir(path, function (err) {
                if (err) {
                    //console.warn("创建" + path + "目录失败!");
                    return false;
                    // return flag;
                }
                else {
                    //console.warn("创建" + path + "目录成功!");
                    return true;
                    // return flag;
                }
            });
...
exports.stat_makeDirSync = function(dirname,mode)
{
   if(fs.existsSync(dirname)){
        return true;
    }else{
        if(this.stat_makeDirSync(path.dirname(dirname), mode)){
...
/*exports.stat_makeDirSync = function (dirpath) {
...
exports.size_between=function (str,min,max) {
    ...
    {
        console.warn("参数"+str+"不为字符串，类型不合法");
        return false;
    }
}
