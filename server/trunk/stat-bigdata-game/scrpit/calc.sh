...
sh calc_msremain.sh $date > $WORKDIR/log/$date/calc_msremain.log 2>&1
...
#task 351
sh calc_vip_active.sh $date > $WORKDIR/log/$date/calc_vip_active.log 2>&1 &
...
#task 21
sh calc_activekeep_left.sh $date > $WORKDIR/log/$date/calc_activekeep.log 2>&1 &
...
#task 19-20
sh calc_newkeep_left.sh $date > $WORKDIR/log/$date/calc_newkeep.log 2>&1 
pids=""
...
php complete.php -1 $date

sh calc_custom_by_game.sh $date > $WORKDIR/log/$date/calc_custom.log 2>&1
