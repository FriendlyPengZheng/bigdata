...
public class RechargeVipReducer extends MapReduceBase implements Reducer<Text, Text, Text, DoubleWritable>
{
    ...
    //output rechargefirst: key=game,zone,server,platform,uid,famt,tamt  value=null
    ...
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, DoubleWritable> output, Reporter reporter) throws IOException
    {
        ...
        if(!payBefore) {
        	outputKey.set(String.format("%s\t%s\t%s\t%s", key.toString(), amt, paySum));
			mos.getCollector("vipfirst", reporter).collect(outputKey, NullWritable.get());
        }
        outputKey.set(String.format("%s\t%s\t%d\t%d\t%d\t%.4f", key.toString(), amt, time, max_time, payCnt, totalSum));
        mos.getCollector("vipall", reporter).collect(outputKey, NullWritable.get());
        ...
        if(payToday && payBefore) {
            ...
            mos.getCollector("vipinterval", reporter).collect(outputKey, NullWritable.get());
        }   
    }
}
