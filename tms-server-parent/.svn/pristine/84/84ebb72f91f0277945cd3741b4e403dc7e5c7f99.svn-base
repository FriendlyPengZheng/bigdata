(function($, undefined) {
    ...
    Module.prototype = {
        ...
        _createInput: function (type, className) {
            return this._createDoc('input', className).attr('type', type);
        },
    ...
    ModuleManage("wrap", {
        options: {
            ...
            ignoreToolsTitle: undefined,
            sharedTitle: undefined,
            grandAllTitle: undefined,
            // 是否提供头部功能
            headEnabled: true,
            // 是否提供底部功能
            bottomEnabled: true,
        ...
        create: function() {
            ...
            this._trigger(this.options.afterCreate, this.content.parent().parent());
        },
        getOption: function(){
            var option = {};
        ...
        refresh: function(option) {
            ...
            if (option && option.multiReload && this.tools && this.tools.multiControl) {
                ...
                if (option && null !== option.searchValue) this.searchValue = option.searchValue;
        ...
        _initEvents: function() {
            ...
            if (this.ignoreTools) {
                this.ignoreTools.on("click", function () {
                    that._trigger(that.options.ignoreTools, that);
                });
            }
            if(this.download) {
            ...
            if (this.search) {
                this.search[1].on("click", function () {
                    var val = $.trim(that.search[2].val());
                    if (val) {
                ...
                this.search[3].on("click", function () {
                    var val = $.trim(that.search[2].val());
                    that.search[2].val("");
                    that.searchValue = "";
                    that.refresh({
            ...
            if (this.grandAll) {
                this.grandAll.on("change", function() {
                    that._trigger(that.options.grandAll, that);
                });
            }
        ...
        _createHeader: function() {
            ...
            if(o.remove){
                this.remove = this._createButton("icon-mod-remove fr ml5")
                    .attr({ "sstid": o.removeTitle }).appendTo(head);
            }
            if(o.download){
                this.download = this._createButton("icon-mod-download fr ml5 stat-button")
                    .attr({
            ...
            if(o.edit){
                this.edit = this._createButton("icon-mod-edit fr ml5")
            ...
            if(o.favor) {
                this.favor = this._createButton("icon-mod-favor fr ml5 stat-button")
                    .attr({
						"title": o.favorTitle,
						"sstid": $.trim(this.options.title)
					}).appendTo(head);
            ...
            // 搜索功能
            if (o.search) {
                var $btn = this._createDoc("i", "search-tag"),
                    $input = this._createDoc("input", "srh-txt"),
                    $search = this._createDoc("span", "fr search-con"),
                    $clear = this._createDoc("i", "search-clear").html("&#215;");
                $search.append($input).append($btn).append($clear).appendTo(head);
                this.search = [$search, $btn, $input, $clear];
            }
            if(o.comment) {
                this.comment = this._createButton("icon-mod-comment fr ml5")
                    .attr("title", o.commentTitle).appendTo(head);
            }
            if (o.ignoreTools) {
                this.ignoreTools = this._createButton("icon-mod-ignore fr ml5")
                    .attr({ title: o.ignoreToolsTitle })
                    .appendTo(head);
            }
            if(o.shared) {
                this.shared = this._createButton("icon-mod-share fr ml5")
            ...
            if (o.calc) {
                ...
                this.calc = this._createButton("icon-mod-calc fr ml5")
                    .attr({ "title": o.calcTitle }).appendTo(head);

                $(min).appendTo(head).addClass('calc-box');
                $(max).appendTo(head).addClass('calc-box');
                $(sum).appendTo(head).addClass('calc-box');
                $(avg).appendTo(head).addClass('calc-box');
            }
            if (o.heatmap) {
                this.heatmap = true;
            ...
            // 总计
            if (o.grandAll) {
                this.grandAll = this._createInput('checkbox');
                if (this.options.grandAllAttr) this.grandAll.attr(this.options.grandAllAttr);
                $(this._createDoc('label', 'mod-calc-row fr')
                    .append(this.grandAll)
                    .append(o.grandAllTitle)
                    ).appendTo(head);
            }
        },
        ...
        _createBody: function() {
            ...
            if (this.options.bottomEnabled) {
                this.table = this._createButton("icon-mod-table").addClass("stat-button").attr({
					'title': "表切换",
					'sstid': this.options.title
				});
                this.graph = this._createButton("icon-mod-graph mr5").addClass("stat-button").attr({
					'title': "图切换",
					'sstid': this.options.title
				});
    ...
    ModuleManage("table", {
        ...
        _createBody: function() {
            ...
            // 创建统计行
            function createAllCalculate () {
                var row = $();
                if(that.options.checkbox){
                    row = row.add(that._createDoc("td", "td hd").text('--'));
                }
                row = row.add(that._createDoc("td", "td hd").text(lang.t('总计')));
                row = row.add(that._createDoc("td", "td hd").attr({
                    colspan: colspan + (that.options.calculate[0] ? that.options.calculate.length : 0)
                }).append(that._createDoc("span", "row-loading").text("loading...")));

                return that._createDoc("tr").attr({
                    'data-all-cal': 1
                }).append(row);
            }
            if (this.options.isAndAllCalacuteRow) list = list.add(createAllCalculate());
    ...
    ModuleManage("hugeProgressiveTable", {
        ...
        _fixedTable : function(container, table){
            if (table.data("statTable")){
                ...
                var fixedColumnNumber = 1;
                this.options.checkbox && fixedColumnNumber ++;
                this.options.cartEntrance && fixedColumnNumber ++;
                table.table({
                    colNum: fixedColumnNumber,
    ...
    ModuleManage("progressiveTable", {
        ...
        getData: function() {
            var that = this,
                ...
                i, j = 0, row, rows,
                length, times;
            this.rows = this.options.container.find("table tbody tr").not('[data-all-cal]');
            length = this.rows.length;
            eachCount = eachCount > length ? length : eachCount;
            // 请求次数，用于统计数据完成度
            times = Math.ceil(length / eachCount);
            while (length > 0) {
                rows = [];
                ids = [];
                ...
                window.setTimeout((function(ids, rows) {
                    return function() {
                        ajax(that.options.url.url, that.getCommonParameters(ids), (function(rows) {
                            return function(res) {
                                if (res.result == 0) {
                                    that._create(rows, res.data, function () {
                                        if (!(-- times)) {
                                            var calculateAllRow = that.options.container.find('table tbody tr').filter('[data-all-cal]');
                                            if (calculateAllRow.length) {
                                                calculateAllRow.find("td:last").remove();
                                                ...
                                                if (that.options.calculate.length) {
                                                    $.each(that.options.calculate, function (i, item) {
                                                        if (item == 'sum') {
                                                            calculateAllRow.append(that._createDoc("td", "td").text(_getSum(that.calculateAllData)));
                                                        } else {
                                                            calculateAllRow.append(that._createDoc("td", "td").text('--'));
                                                        }
                                                    });
                                                }
                                                $.each(that.calculateAllData, function (i, item) {
                                                    calculateAllRow.append(that._createDoc("td", "td").text(formatFloat(item, 2)));
                                                });
                                            }
                                            that._trigger(that.options.afterAllLoaded);
                                        }
                                    });
                                }
                            };
                    ...
                    window.setTimeout((function(row, data) {
                        return function() {
                            ...
                            $(data.data.reverse()).each(function(i, item) {
                                ...
                                that.calculateAllData[i] !== undefined || (that.calculateAllData[i] = 0);
                                if (item !== null) {
                                    seq.push(parseFloat(item));
                                    that.calculateAllData[i] += parseFloat(item);
                                }
	...
	_createPagination: function () {
            ...
            this._getPaginationData(function (count) {
            	//console.log("friendly+++++++++++++++++++comming in _getPaginationData");
        ...
        _create: function(data){
            ...
            this.getData(function(data){
                that.container.empty().removeClass("flash-loading");
        ...
        _getAjaxData: function(callback) {
            var that = this;
            ajax(this.options.url.extend, $.extend(this.getPageParameters(), this.params), function(res) {
                if (res.result == 0) {
                    that._trigger(callback, res.data);
                } else {
                    that.options.container.empty().addClass("mod-no-data").text(that.options.ajaxErrorHit);
