...
function _getPlatformId(){
	...
	//console.log("=333&&&&&&platform_id:" + platform_id);
    return $("#J_platform").find(":selected").attr("data-id");
}

function _getZSId(){
	//zs_id = $("#J_zoneServer").find(".selected-item").prop("data-id");
	//console.log("2&&&&&&zs_id:" + zs_id);
	//console.log("_getZSId(444)!!!");
    return $("#J_zoneServer").find(".selected-item").attr("data-id");
    //return $("#J_zoneServer").attr("tabindex");
	//allElements = document.getElementById("J_zoneServer").getElementsByTagName("li");
	//console.log("~~~11len:~~~~~" + allElements);
	//for (var i=0; i< allElements.length; i++ ) {
	//	console.log("className:" + allElements[i].className);
	//}
	
	//var allElements = $("#J_zoneServer").children();
	//console.log("~~444~~~allElements:~~~~~" + JSON.stringify(allElements));
	//console.log("~~~J_zoneServer:~~~~~" + JSON.stringify($("#J_zoneServer")[0]));
	//console.log("~~~allElements li:~~~~~" + JSON.stringify());
	//return 0;
}
/**
 * @brief _zoneFac
...
window.getPageParam = function(flag){
	...
	console.log("zs_id:" + _getZSId());
	console.log("platform_id:" + _getPlatformId());
	if (flag === "download") {
	    ...
	    _getRateConfig = function (container, title, id, type) {
            return [{
                ...
                child: [{
                    type: "data",
                    url: {
                        extend: ["", "../../mission/getMissionDetail?mission_type=" + (type ? type : "main")
                                 + "&sstid=" + id
                                 + "&rate=" + 1],
                        page: function () {
                            return getPageParam();
...
function _getTrendConfigByMission(container, title, id, type) {
    return [{
        ...
        child: [{
            type: "data",
            url: {
                extend: ["", "../../mission/getMissionDetail?mission_type="
