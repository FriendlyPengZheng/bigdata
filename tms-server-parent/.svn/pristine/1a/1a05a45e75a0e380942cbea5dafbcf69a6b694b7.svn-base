 ...
 * 
 */
public class HdfsTmsDataSendTask extends AbstractTask<TmsStreamer> {
	...
	private Logger LOG=LoggerFactory.getLogger(HdfsTmsDataSendTask.class);
	
	public HdfsTmsDataSendTask(int state, CountDownLatch cdl,LogMgrService logMgrService) {
		setState(state);
...
*/			System.out.println("bean:" + logMgrService);
			...
			while ((line = br.readLine()) != null) {
				...
				if (lines.length == 4) {
					...
					try {
						Double value = Double.valueOf(lines[3]);
						datatoObject = new DataToObject(
