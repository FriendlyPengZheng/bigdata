...
public class SetExpressionAnalyzer {
	...
	private HashMap<String, Boolean> initTfMap() {
		tfMaps.clear(); 
		if(this.setExpressionInProcess == null)return tfMaps;
		boolean[][] tfMatrix = getAllPossibleTF(getNumOfSet());
		for(int j = 0;j<=tfMatrix.length-1;j++){
			...
			try {
				tfMaps.put(tfKey, Boolean.valueOf(engine.eval(convertedExpressionInProcess).toString()));
			} catch (ScriptException e) {
				e.printStackTrace();
			}
		}
		flags = new Boolean[getNumOfSet()];
		return tfMaps;
	}
	...
	/*
	 * 判断传入的flags是否满足该集合表达式
	 */
	public boolean isInSet(Iterator it) {
		...
		while(it.hasNext()){
			...
			if(o instanceof Integer){
				flags[setMap.get("["+o+"]")] = true;
			}else{
				flags[setMap.get(o)] = true;
			}
		}
		return tfMaps.get(getTfStr(flags));
	}
	
	private String getTfStr(Boolean[] flags) {
		sb.setLength(0);
		for(int i = 0 ;i<flags.length;i++){
			sb.append(flags[i]);
		}
...
//		analyzer.analysis("m_1[d0]∩a_12[d2]-m_2[0]");
		analyzer.analysis("[1]∩[2]-[0]");
		HashSet set = new HashSet();
//		set.add("m_1[d0]");
//		set.add("a_12[d2]");
		set.add(1);
		set.add(0);
		System.out.println(analyzer.isInSet(set.iterator()));
	}
