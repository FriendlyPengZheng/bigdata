...
public class HourCaculateDataTools extends AbstactCaculateDataTools{
	 ...
	 * @param dao接口
	 */
	public HourCaculateDataTools(String  expr, 
		...
		m_exprName = m_request.getParameter("data_info[" + m_index + "][data_name]");
		m_logMgrService = logMgrService;
		m_unit = m_request.getParameter("data_info[" + m_index + "][unit]");
		String precisionString = m_request.getParameter("data_info[" + m_index + "][precision]");
	...
	protected ArrayList<Map<String, Double>> preDataList(List<String> keyList) {
		// 遍历index, 取出数据
		// 时间序列数据
	    // System.out.println("pre keyList: " + keyList);
		...
		//serverId = getServerId(gameId, platformId, zoneId, serverId, m_logMgrService);
		...
		for (int i = 0; i < indexList.size(); ++i) {
			...
			try {
		        ...
		        System.out.println("data_info_name" + "data_info[" + indexList.get(i) + "][data_id]");
				System.out.println("dataIdString:" + dataIdString);
				...
				periodString = (periodString == null || periodString.isEmpty()) ? "-1" : periodString;
				...
				// resultInfos = m_logMgrService.getDateValueHour(dataId, serverId, keyList.get(0), keyList.get(keyList.size() - 1), 0);
				// 时间区间需要重置，周数据处理有些差异
				...
				// resultInfos = m_logMgrService.getValueInfo(dataId, 10, fromDateString, toDateString, period);
				// 测试代码
//				if (!m_logMgrService.jedisClientInit()) {
//					// redis客户端初始化失败，return 
//					return null;
//				}
				...
				resultInfos = m_logMgrService.getDateValueHour(dataId, gameId, serverId, fromDateString);
				// System.out.println("min data");
				System.out.println("resultInfo from db Hour: " + resultInfos);
