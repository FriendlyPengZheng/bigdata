...
function Datepicker() {
	this._defaults = {
        ...
        autoWidth: false, //True to set the width of the input target, max width = 300px
...
$.extend(Datepicker.prototype, {
	...
	commands: { // Command actions that may be added to a layout by name
		...
		close: {text: 'closeText', status: 'closeStatus', // Close the datepicker
			...
			date: function(inst) { return null; },
			action: function(inst) {
                var onOK = inst.get('onOK');
                if (onOK) {
                    onOK.apply(this, [inst.selectedDates]);
                }
                $.datepick.hide(this);
            }
		},
        ...
        //candice 2013-12-10
        oneDay: {text: 'oneDayText', status: 'oneDayStatus', // today
			//keystroke: {keyCode: 38, ctrlKey: true}, // Ctrl + Up
			enabled: function(inst) { return true; },
			date: function(inst) { return $.datepick.today(); },
			action: function(inst) {
                inst.selectedDates = [$.datepick.today(), $.datepick.today()];
                $.datepick._updateInput((inst.target)[0]);
                $.datepick.hide( inst );
            }
        },
        yesterday: {text: 'yesterdayText', status: 'yesterdayStatus', // yesterday
			//keystroke: {keyCode: 38, ctrlKey: true}, // Ctrl + Up
			enabled: function(inst) { return true; },
			date: function(inst) { return $.datepick.add($.datepick.today(), -1, 'd'); },
			action: function(inst) {
                inst.selectedDates = [$.datepick.add($.datepick.today(), -1, 'd'), $.datepick.add($.datepick.today(), -1, 'd')];
                $.datepick._updateInput((inst.target)[0]);
                $.datepick.hide( inst );
            }
        },
        sevenDays: {text: 'sevenDaysText', status: 'sevenDaysStatus', // nearly 7 days
			//keystroke: {keyCode: 38, ctrlKey: true}, // Ctrl + Up
			enabled: function(inst) { return true; },
			date: function(inst) { return $.datepick.add($.datepick.today(), -7, 'd'); },
			action: function(inst) {
                inst.selectedDates = [$.datepick.add($.datepick.today(), -7, 'd'), $.datepick.add($.datepick.today(), -1, 'd')];
                $.datepick._updateInput((inst.target)[0]);
                $.datepick.hide( inst );
            }
        },
        fourteenDays: {text: 'fourteenDaysText', status: 'fourteenDaysStatus', // nearly 14 days
			//keystroke: {keyCode: 38, ctrlKey: true}, // Ctrl + Up
			enabled: function(inst) { return true; },
			date: function(inst) { return $.datepick.add($.datepick.today(), -14, 'd'); },
			action: function(inst) {
                inst.selectedDates = [$.datepick.add($.datepick.today(), -14, 'd'), $.datepick.add($.datepick.today(), -1, 'd')];
                $.datepick._updateInput((inst.target)[0]);
                $.datepick.hide( inst );
            }
        },
        thirtyDays: {text: 'thirtyDaysText', status: 'thirtyDaysStatus', // nearly 31 days
			//keystroke: {keyCode: 38, ctrlKey: true}, // Ctrl + Up
			enabled: function(inst) { return true; },
			date: function(inst) { return $.datepick.add($.datepick.today(), -31, 'd'); },
			action: function(inst) {
                inst.selectedDates = [$.datepick.add($.datepick.today(), -31, 'd'), $.datepick.add($.datepick.today(), -1, 'd')];
                $.datepick._updateInput((inst.target)[0]);
                $.datepick.hide( inst );
            }
        }
	...
	defaultRenderer: {
		...
		pickerClear: '<div class="datepick">' +
            '<div class="datepick-nav">{link:prev}{link:next}</div>{months}' +
		'{popup:start}<div class="datepick-ctrl">{link:clear}{link:close}</div>{popup:end}' +
		'<div class="datepick-clear-fix"></div></div>',
		pickerBottomDisabled: '<div class="datepick">' +
            '<div class="datepick-nav">{link:prev}{link:next}</div>{months}' +
		'{popup:start}{popup:end}' +
		'<div class="datepick-clear-fix"></div></div>',
        pickerShortCut : '<div class="datepick">' +
            '<div class="datepick-nav">{link:prev}{link:oneDay}{link:yesterday}{link:sevenDays}{link:fourteenDays}{link:thirtyDays}{link:next}</div>{months}' +
		'{popup:start}{popup:end}' +
		'<div class="datepick-clear-fix"></div></div>',
		...
		monthNoCalender: '<div class="datepick-month"><div class="datepick-month-header">{monthHeader}</div></div>',
        ...
        // Class for currently selected range dates
		rangeSelectedClass: 'datepick-range-selected',
	...
	_attachPicker: function(target, settings) {
		...
		var inst = {
			...
			drawDate: null,
			pickingRange: false,
			inline: ($.inArray(target[0].nodeName.toLowerCase(), ['div', 'span']) > -1),    // boolean
			get: function(name) {    // Get a setting value, defaulting if necessary
                ...
                if ($.inArray(name, ['defaultDate', 'minDate', 'maxDate']) > -1) { // Decode date settings
                    if (!this.get("rangeSelect") || name !== "defaultDate") {
                        value = $.datepick.determineDate(value,
														 null,
	...
	_attachments: function(target, inst) {
		...
		if (inst.get('selectDefaultDate') && inst.get('defaultDate') &&
            ...
            if (inst.get("rangeSelect")) {
                this.setDate(target[0], $.datepick._extractDates(inst, inst.get("defaultDate")));
            } else {
	...
	show: function(target) {
		...
		if (inst) {
			...
			var zIndex = $(target).css('zIndex');
			zIndex = (zIndex == 'auto' ? 0 : parseInt(zIndex, 10)) + 300;
		...
		if (inst) {
			...
			if (inst.inline || $.datepick.curInst == inst) {
				...
				if (onChange && (!inst.prevDate ||
                    ...
                    //only month and year by candice
                    inst.selectedDates = [this.retriveMonth(inst.drawDate.getFullYear(), inst.drawDate.getMonth())];
	...
	_keyDown: function(event) {
		...
		if (inst.div) {
			if (event.keyCode == 9) { // Tab - close
				...
				var tRange = $(event.target).val().split('~');
				var from = new Date(), to = new Date();
				from.setFullYear(tRange[0].split('-')[0], tRange[0].split('-')[1] - 1, tRange[0].split('-')[2]),
				to.setFullYear(tRange[1].split('-')[0], tRange[1].split('-')[1] - 1, tRange[1].split('-')[2]);
				inst.selectedDates = [];
				inst.selectedDates.push(from);
				inst.selectedDates.push(to);
				$.datepick.hide(target);
				$(event.target).blur();
				...
				for (var name in commands) {
					...
					if (command.keystroke && command.keystroke.keyCode == event.keyCode &&
    ...
    retriveMonth: function(year, month){
        var d = new Date();
        d.setFullYear(year);
        d.setMonth(month-1);
        return d;
	...
	selectDate: function(target, elem) {
		...
		if (inst && !this.isDisabled(target)) {
			...
			this._updateInput(target);
            //set the width of target, maxWidth = '300' candice 2014-3-27
            if(inst.get('autoWidth')){
                var width = ($(target).val()).length * 7;
                $(target).width(width > 300 ? 300 : ( width < 70 ? 70 : width ));
            }
	...
	_generateContent: function(target, inst) {
        ...
        // edit by violet , make the month selected the first to show
        ...
        //candice
        //rules: 1) need shortcut buttons when shortCut = true => render.pickerShortCut
        //       2) need {clear} when multiSelect != 0 => renderer.pickerClear
        //       3) hide {close} when bottomDisabled = true => renderer.pickerBottomDisabled
        //       4) normal : has {close}
		var p = inst.settings.shortCut
            ? renderer.pickerShortCut
            : ( inst.settings.multiSelect
                ? renderer.pickerClear
                : (inst.settings.bottomDisabled ? renderer.pickerBottomDisabled : renderer.picker));
        var picker = this._prepare(p, inst).replace(/\{months\}/, monthRows).
	...
	_generateMonth: function(target, inst, year, month, renderer, first) {
		...
		for (var week = 0; week < numWeeks; week++) {
			...
			for (var day = 0; day < 7; day++) {
				...
				if (rangeSelect && inst.selectedDates.length > 0) {
					selected = (drawDate.getTime() == inst.selectedDates[0].getTime() ||
						drawDate.getTime() == inst.selectedDates[1].getTime());
					rangeSelected = (drawDate.getTime() > inst.selectedDates[0] &&
				...
				days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
					...
					(rangeSelected && (selectOtherMonths || drawDate.getMonth() + 1 == month) ?
					' ' + renderer.rangeSelectedClass : '') +
        ...
        //rules: hide day calender when hideCalendar is true ( only show month and year)
        var monthRenderer = inst.settings.hideCalendar ? renderer.monthNoCalender : renderer.month;
		var monthHeader = this._prepare(monthRenderer, inst).match(/\{monthHeader(:[^\}]+)?\}/);
		monthHeader = (monthHeader[0].length <= 13 ? 'yyyy MM' :
		...
		return this._prepare(monthRenderer, inst).replace(/\{monthHeader(:[^\}]+)?\}/g, monthHeader).
...
(function($) {
	$.datepick.regional['zh-CN'] = {
		...
		renderer: $.extend({}, $.datepick.defaultRenderer,
			{month: $.datepick.defaultRenderer.month.replace(/monthHeader/, 'monthHeader:MM yyyy年')}),
        ...
        prevWeekText: '上周', prevWeekStatus: '显示上周',
        oneDayText: '今日', oneDayStatus: '显示今日',
        yesterdayText: '昨日', yesterdayStatus: '显示昨日',
        sevenDaysText: '近7日', sevenDaysStatus: '显示近7日',
        fourteenDaysText: '近14日', fourteenDaysStatus: '显示近14日',
        thirtyDaysText: '近31日', thirtyDaysStatus: '显示近31日',
		clearText: '清除', clearStatus: '清除已选日期',
		closeText: '确定', closeStatus: '确定当前选择',
