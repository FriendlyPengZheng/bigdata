package com.taomee.tms.common.schedule.job;
...
import com.taomee.tms.common.schedule.ThreadPool;
...
public abstract class AbstractJob<T> implements IJob<T> {
	...
	public boolean start() {
		...
		if (state == 1) {
			...
			if (scheduleExecutorService != null
					&& !scheduleExecutorService.isShutdown())
				res = stop();
			else {
				run();
				return true;
	...
	protected void before() {
		scheduleExecutorService = Executors
				.newScheduledThreadPool(scheduleSize);
	...
	/**
	 * TaskTimer是一个继承AbstractTask类，同时这个类也是继承Runnable接口的一个抽象类，实际执行的是TaskTimer的run方法里面的内容
	 */
	private void run() {
		before();
		scheduleExecutorService.scheduleWithFixedDelay(
				new TaskTimer(this.state), initialDelay4f, delay4f,
				TimeUnit.MILLISECONDS);
		log.info("【" + getClass().getName() + " start successful，Time: "
				+ DateUtils.getNowDateHms() + "】");
		after();
	}
	...
	public boolean stop() {
		...
		if (!scheduleExecutorService.isTerminated()) {
			try {
				boolean wt = scheduleExecutorService.awaitTermination(5,
						TimeUnit.SECONDS);
				if (wt)
				...
				log.error("Can not stop " + getClass().getName() + " Thread: "
						+ e.getMessage());
			}
		} else
			scheduleExecutorService.shutdownNow();
		recycle();
		res = true;
		log.info("【" + getClass().getName() + " stop successful，Time: "
				+ DateUtils.getNowDateHms() + "】");
		return res;
	}
		...
		public void process() {
			try {
				if (dataQueue == null || dataQueue.isEmpty()) {
					log.debug("no data need to process, time:"
							+ DateUtils.getNowDateHms());
					return;
				}
				if (taskQueue == null || taskQueue.isEmpty()) {
					log.debug("no free process unit, time:"
							+ DateUtils.getNowDateHms());
					return;
				}
				while (!taskQueue.isEmpty()) {
					T t = dataQueue.take();//从队列消费data，当队列为空，会阻塞住
					int s = beforex(t);
					if (s == status.s_1.c) {
						ITask<T> task = taskQueue.take();//从队列消费task，当队列为空，会阻塞
						task.init(t);
				...
				log.error("【" + getClass().getName()
						+ " cause unkonw exception: " + e.getMessage()
						+ ", Time: " + DateUtils.getNowDateHms() + "】");
			}
		}
	}
	...
	public enum status {
		s_1(1, "Default"), s_2(2, "NextItem"), s_3(3, "TempItem");
