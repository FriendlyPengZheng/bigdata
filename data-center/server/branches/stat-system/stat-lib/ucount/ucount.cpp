...
static int merge_uvalue_intersect(uint32_t ucount, uint32_t isset, void *p_user_data)
{
	void **pp_user_data = (void **)p_user_data;
	i_ucount *p_ucount = (i_ucount *)pp_user_data[0];
	i_uvalue *p_uvalue = (i_uvalue *)pp_user_data[1];

	if (p_uvalue->get(&ucount, sizeof(ucount), NULL, NULL) != 0) {
		if (p_ucount->set(ucount, i_ucount::UNSET) != 0) {
			return -1;
		}
	}
...
static int merge_uvalue_except(uint32_t ucount, uint32_t isset, void *p_user_data)
{
	void **pp_user_data = (void **)p_user_data;
	i_ucount *p_ucount = (i_ucount *)pp_user_data[0];
	i_uvalue *p_uvalue = (i_uvalue *)pp_user_data[1];
...
static int merge_uvalue_union(const void *p_key_data, uint32_t key_size, 
		...
		void *p_user_data)
{
	...
	uint32_t ucount = *(uint32_t *)p_key_data;
	if (ucount <= i_ucount::UFILE_SIZE * 8) {
		if (p_ucount->set(ucount, i_ucount::SET) != 0) {
			return -1;
		}
	}
...
int c_ucount::merge(i_uvalue *p_uvalue, key_opcode_t key_opcode)
{
	void *pp_user_data[2] = {this, p_uvalue};

	switch (key_opcode) {
		case INTERSECT:
			if (traverse(merge_uvalue_intersect, pp_user_data, 1) != 0) {
				ERROR_LOG("ERROR: traverse(merge_uvalue_intersect, pp_user_data, 1) != 0");
				return -1;
			}
			break;
		case EXCEPT:
			if (traverse(merge_uvalue_except, pp_user_data, 1) != 0) {
				ERROR_LOG("ERROR: traverse(merge_uvalue_except, pp_user_data, 1) != 0");
				return -1;
			}
			break;
		case UNION:
			if (p_uvalue->traverse(merge_uvalue_union, pp_user_data) != 0) {
				ERROR_LOG("ERROR: p_uvalue->traverse(merge_uvalue_union, pp_user_data) != 0");
				return -1;
			}
			break;
		default:
