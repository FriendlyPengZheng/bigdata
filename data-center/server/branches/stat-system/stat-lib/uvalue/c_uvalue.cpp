...
int user_compare(DB *db, const DBT *key1, const DBT *key2)
{
...
static int merge_ucount_union(uint32_t ucount, uint32_t isset, void *p_user_data)
{
	if (p_user_data == NULL) {
		return -1;
	}
	...
	i_uvalue *p_uvalue = (i_uvalue *)pp_user_data[0];

	uint32_t value = 1;
	if (p_uvalue->set(&ucount, 4, &value, 4, NULL) != 0) {
		return -1;
	}
	return 0;
}
...
int c_uvalue::merge(i_ucount *p_ucount, key_opcode_t key_opcode)
{
    ...
    {
		void *p_user_data[2] = {this, p_ucount};
		if (p_ucount->traverse(merge_ucount_union, p_user_data, 1) != 0) {
			return -1;
		}
    }
    else 
