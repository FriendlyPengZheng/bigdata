...
public class IPCountryDistr
{
    ...
    // if <ip> in network byte order (big endian), convert to host byte order
    // <bigendian> specify ip if in big endian byte order? true in big endian, false not in
    public IPRange getHitRange(long ip, boolean bigendian)
    {
        int low = 0;
        int high = ipRangeList.length - 1;
        int mid = 0;
        long ipHost = bigendian ? this.convertToIPHost(ip) : ip;
    ...
    private static String unknownCountryName = "未知国家";
    private static String unknownCountryCode = "0";

    public String getIPCountryCode(long ip)
    {
        IPRange ipRange = getHitRange(ip, true);

        if (ipRange == null) { return unknownCountryCode; }
    ...
    public String getIPCountryName(long ip)
    {
        ...
        if (countryCode.equals(unknownCountryCode)) { return unknownCountryName; }

        String countryName = this.countryNameMap.get(countryCode);
        if (countryName == null) { return unknownCountryName; }
