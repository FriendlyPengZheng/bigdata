...
public class LoadPv extends LoadAds
{
	...
	if(tad.lastIndexOf('#') != 0)
	{
        ...
        if (adid == null)
       	{
            adid = insertAd(tad);
            if (adid == IGNORE_ERROR_CODE) { return null; }
        }
	...
	if(url.lastIndexOf('#') != 0)
	{
        ...
        if (urlid == null) {
            urlid = insertUrl(url);
            if (urlid == IGNORE_ERROR_CODE) { return null; }
        }
    ...
    {
        int ret = ToolRunner.run(new Configuration(), new LoadPv(), args);
        System.exit(ret);
    }
