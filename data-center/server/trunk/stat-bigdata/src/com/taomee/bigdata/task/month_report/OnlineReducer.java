...
public class OnlineReducer extends MapReduceBase implements Reducer<Text, Text, Text, Text> //不会同一分钟发多条在线人数，不需要combiner
{
    ...
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException
    {
		...
		//System.out.println(String.format("[debug] n = %d", n));
        for(Integer i=0; i<cOnlineCnt.length; i++) {
            pcu = cOnlineCnt[i] > pcu ? cOnlineCnt[i] : pcu;
            if(cOnlineCnt[i] != 0) {
				//System.out.println(String.format("[debug] cOnlineCnt[%d] = %d", i,cOnlineCnt[i]));
                acuSum += Long.valueOf(cOnlineCnt[i]);
                acuCount ++;
            }
        }
		//System.out.println(String.format("[debug] acuSum = %d", acuSum));
