...
{
    if(type == PROC_MAIN){
        BOOT_LOG(0,"config-server BuildTime: %s %s",__TIME__,__DATE__);
        ...
        if(get_work_idx() == 0){
            DEBUG_LOG("stat-config-control BuildTime: %s %s",__TIME__,__DATE__);
            set_title("stat-config-control");
            //初始化与中心服务器的连接
            main = new (std::nothrow)StatLogControl();
            if(main == NULL || main->init() != 0){
                return -1;
            }
            DEBUG_LOG("stat-config-control init successfully");
        }else{
            DEBUG_LOG("stat-config-server BuildTime: %s %s",__TIME__,__DATE__);
            set_title("stat-config-server");
            main = new (std::nothrow)StatConfigServer();
            if(main == NULL || main->init() != 0){
                return -1;
            }
            DEBUG_LOG("config-server init successfully");
        }
    }
