...
public class ModelHourSumReducer extends MapReduceBase implements Reducer<Text, IntWritable, Text, IntWritable>
{
    ...
    public void reduce(Text key, Iterator<IntWritable> values, OutputCollector<Text, IntWritable> output, Reporter reporter) throws IOException
    {
        ...
        for(int i=max; i>=1; i--) {
            ...
            mos.getCollector(hour+"part", reporter).collect(outputKey, outputValue);

            outputKey.set(String.format("SET\t%s\t-1\t-1\t-1\t_%smodel_\t_%smodel_\tstep,percent\t%d:%s",
                        game, modelid, modelid, i, getStepName(game, modelid, i)));
            doubleOutputValue.set(stepscnt[i]/sum*100.0);
            mos.getCollector(hour+"percent", reporter).collect(outputKey, doubleOutputValue);
            ...
            if(i != max) {
                ...
                mos.getCollector(hour+"percentbystep", reporter).collect(outputKey, doubleOutputValue);
            }
        }
        outputKey.set(String.format("SET\t%s\t-1\t-1\t-1\t_%smodel_\t_%smodel_\tstep,percentbystep\t%d:%s",
