...
int StatAppPushAlarmUnregister::proc_proto(int fd, const void* pkg)
{

    if(m_stat_app_push == NULL)
        return -1;

    StatAppLoginProto::StatAppLogoutRequest req;
	...
	DEBUG_LOG("unregister.user_name:%s, token:%s",
			req.user_name().c_str(), req.token().c_str());

    int ret = m_stat_app_push->app_unregister(req.user_name(), req.token().c_str());
	...
	{
		ERROR_LOG("buffer is not enough when sending StatAppLogoutResponse, need %lu buf buffer size is %lu",
	...
	DEBUG_LOG("unregister reponse.len:%d ret:%u", ret_pkg->len, res.ret());
