...
int send_rqst(int fd, const char *p_rqst_msg, char *p_resp_buf, int buf_len, int timeout)
{
	int rv = -1;
	int rqst_msg_rmng_len = *(uint32_t *)p_rqst_msg;
		...
		if (rv < 0) {
			...
			if (FD_ISSET(fd, &read_set)) {
				...
				if (rv == -1 && errno != EAGAIN ) {
					...
					if (resp_buf_used_len >= (int)sizeof(uint16_t) && 
						resp_buf_used_len >= *(uint16_t *)p_resp_buf) {
