...
$(".upd-btn").on("click", function() {
    var t = $(this),
        ...
        show = function() {
            ...
            $("#J_setIgnoreBtn").data("data", values.ignore);
            $("#J_setFuncBtn").data("data", values.func_slot);
            _getIgnoredComponents(function() {
                _getFuncSlot(function() {
                    var ignore = values.ignore.split("_"), ignoreTitle = [],
                        funcSlot = values.func_slot, funcTitle = [];
...
$(".del-btn").on("click", function() {
    ...
    DELDIALOG = $.Dlg.Util.popup({
        ...
        save: function(con) {
            ajax(getUrl("common", "game", "setStatus"), {
...
function _save(param) {
    overlayer({ text: "加载中..."});
    ajax(getUrl("common", "game", "save"), param, function (res) {
        if (res.result == 0) {
            hidelayer("加载成功~.~");
            location.reload();
        } else {
...
function _getGameType(callback) {
    if(ITEMS.GAME_TYPE.length) {
        ...
        ajax(getUrl("common", "game", "getGameType"), null, function(res) {
            if (res.result == 0) {
...
function _getStatus(callback) {
    if(ITEMS.STATUS.length) {
        ...
        ajax(getUrl("common", "game", "getStatus"), null, function(res) {
            if (res.result == 0) {
...
function _getIgnoredComponents(callback) {
    if(ITEMS.IGNORED_COMPONENT.length) {
        ...
        ajax(getUrl("admin", "manage", "getIgnoredComponents"), null, function(res) {
            if (res.result == 0) {
                $.each(res.data, function() {
                    ITEMS.IGNORED_COMPONENT.push({
                        title: this.component_title + "（" + this.ignore_id + "）",
                        value: this.ignore_id
                    });
                });
            }
            callback();
...
function _getFuncSlot(callback) {
    if(ITEMS.FUNC_SLOT.length) {
        ...
        ajax(getUrl("common", "game", "getFuncMask"), null, function(res) {
            if (res.result == 0) {
