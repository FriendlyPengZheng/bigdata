			...
			$.Dlg.Util.confirm(lang.t("是否发送？"), lang.t("点击确定按钮开始发送..."), function() {
				...
				selected.each(function () {
					...
					case "1":
						...
						break;
					case "2":
						ajax(getUrl("admin","Email","sendWeixin"), {
							email_id : emailId,
							is_test : is_test
						}, function(res) {
							if (res.result != 0) {
    ...
    t.find(".threshold").click(function() {
        ...
        THRESHOLDDIALOG = $.Dlg.Util.popup({
            ...
            contentHtml: $("<form>").append($('<label class="lable-inline">'
											  ...
											  + '</label>'))
				.append((new tm.form.fieldSet({items: items})).getElement()),
            save: function(con) {
...
function _get_param(t){
    var aIdExpr = (t.attr("data-expr")).split("|"),//("A,B|{0}/{1}").split("|");
        ids = aIdExpr[0].split(";"),
        param = {
            gpzs_id: t.attr("data-gpzs"),
            "exprs[0][expr]": aIdExpr[1],
            "exprs[0][data_name]": t.attr("data-name")
        }, tmpId;
    for (var i = 0; i < ids.length; i ++) {
        tmpId = ids[i].split('_');
        if (tmpId.length == 2) {
            param["data_info[" + i + "][data_id]"] = tmpId[0];
            param["data_info[" + i + "][gpzs_id]"] = tmpId[1];
        } else {
