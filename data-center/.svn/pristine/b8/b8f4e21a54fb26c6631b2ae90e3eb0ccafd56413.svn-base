...
{
	printf("job_id=%d, fpath=%s, fname=%s, glen=%d\n",
...
void StatMrGenerate::load_rewrite_file()
{
	...
	string command = "| grep -v Found | awk -F \"/\" '{print $NF}' |awk -F \"-\" '{print $1}'`";
...
void StatMrGenerate::load_jobinfo()
{
	...
	oss << "com.taomee.bigdata.driver.MultipleInputsJobDriver" << " \\\n" << '\t';
...
int StatMrGenerate::get_jobinfo(const int job_id)
{
	...
	sprintf(fields, "job_id,job_name,gameInfo,jar,params,conf,outKey,outValue, \
	...
	else
	{
		...
		jobinfo.jar = row[3];
		jobinfo.params = row[4];
		jobinfo.conf = row[5];
		jobinfo.outKey = row[6];
		jobinfo.outValue = row[7];
		jobinfo.inFormat = row[8];
		jobinfo.outFormat = row[9];
		jobinfo.inputPath = row[10];
		jobinfo.outputPath = row[11];
		jobinfo.combinerClass = row[12];
		jobinfo.reducerClass = row[13];
		jobinfo.mos = row[14];
		jobinfo.dbUpload_id = row[15];
		jobinfo.mysqlUpload_id = row[16];
		string temp = row[17];
		jobinfo.fix_flag = atoi(temp.c_str());
		return 0;
	}
}
