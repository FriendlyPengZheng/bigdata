    ...
    public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException
    {
        ...
        if(logAnalyser.analysis(value.toString()) == ReturnCode.G_OK ) {
            ...
            if(game != null &&
                ...
                uid != null &&
                golds != null) {
                if(sstid.compareTo("_usegold_") == 0) {
                    outputKey.set(String.format("%s\t%s\t%s\t%s\t%s",
                                game, zone, server, platform, Distr.getDistrName(coinsDistr, Distr.getRangeIndex(coinsDistr, Double.valueOf(golds).intValue()), 100)));
                    outputValue.set(String.format("%s\t%s\t1", golds, uid));
