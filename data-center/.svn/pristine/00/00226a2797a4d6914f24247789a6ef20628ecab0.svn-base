        ...
        implements Tool
{
    ...
    public int run(String[] args) throws IOException
    {
		...
		HashSet<Integer> gameId = new HashSet<Integer>();
                ...
                if (args[i].equals("-inFormat")) {
					...
					for(int j = 0; j < gameinfo.length; j++) {
						try{
							...
							gameId.add(Integer.valueOf(gameinfo[j]));
					...
					Iterator<Integer> it = gameId.iterator();
					while(it.hasNext()){
						String gid = it.next().toString();
						mosNames.add(valItems[0] + "G" + gid);
					...
					//add mosname "Gother"
                    mosNames.add(valItems[0] + "Gother");
                    mosOutputFormats.add(
                            Class.forName(valItems[1]).asSubclass(OutputFormat.class));
                    mosOutputKeyClasses.add(
                            Class.forName(valItems[2]).asSubclass(WritableComparable.class));
