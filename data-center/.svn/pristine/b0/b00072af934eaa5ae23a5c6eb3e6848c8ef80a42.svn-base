...
public class RechargePayReducer extends MapReduceBase implements Reducer<Text, Text, Text, DoubleWritable>
{
    ...
    //output rechargefirst: key=game,zone,server,platform,uid,famt  value=null
    //output rechargeall: key=game,zone,server,platform,uid,famt,ftime,ltime,cnt,tamt  value=null
    //output rechargeinterval: key=game,zone,server,platform,uid,intervaldays  value=null
    public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, DoubleWritable> output, Reporter reporter) throws IOException
    {
        ...
        if(!payBefore) {
			...
			mos.getCollector("rechargefirst", reporter).collect(outputKey, NullWritable.get());
        }
        outputKey.set(String.format("%s\t%s\t%d\t%d\t%d\t%.4f", key.toString(), amt, time, max_time, payCnt, totalSum));
        mos.getCollector("rechargeall", reporter).collect(outputKey, NullWritable.get());
        if(payToday && payBefore) {
            ...
            mos.getCollector("rechargeinterval", reporter).collect(outputKey, NullWritable.get());
        }
    }
}
