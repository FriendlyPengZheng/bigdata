...
int c_work_proc::message_process(ps_message_t *ps_message, int error_index)
{
    ...
    while(p_msg_trans != NULL) {
        ...
        if (pro_iter == m_proto_map.end()) {
			...
			// TODO: Temp hard code, remove them later.
			map<uint32_t, proto_so_t>::iterator tmpit = m_proto_map.find(p_msg_trans->proto_id + 100);
			if((tmpit != m_proto_map.end())
					&& ((tmpit->second).proto_process(ss_msg_hdr, msg_buffer) != 0)) {
				ERROR_LOG("proto process 2 failed, proto_id: %d, report_id: %d",
            	            p_msg_trans->proto_id, p_msg_trans->report_id);
                return p_msg_trans->arg_index + 1;
			}
        }
