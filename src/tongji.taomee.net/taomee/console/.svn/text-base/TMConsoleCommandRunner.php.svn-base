<?php
/**
 * TMConsoleCommandRunner manages commands and executes the requested command.
 *
 * @property string $scriptName The entry script name.
 * @property TMConsoleCommand $command The currently active command.
 */
class TMConsoleCommandRunner extends TMComponent
{
    /**
     ...
     * {@link TMBase::getPathOfAlias class path alias} of the command.
    ...
    /**
     * @var string The entry script name.
     */
    private $_scriptName;

    /**
     * @var TMConsoleCommand The currently active command.
     */
    private $_command;
     ...
     * @return TMConsoleCommand the command object. Null if the name is invalid.
     */
        ...
        if ($command !== null) {
            if (is_string($command)) { // class file path or alias
                if (strpos($command, '/') !== false) {
                    ...
                    $className = TM::import($command);
                }
                return new $className($name, $this);
            } else { // an array configuration
                $oCommand = TM::createComponent($command);
                $oCommand->setName($name)->setCommandRunner($this);
                return $oCommand;
            }
        } elseif ($name === 'help') {
            return new TMHelpCommand('help', $this);
        } else {
            return null;
        }
    }
