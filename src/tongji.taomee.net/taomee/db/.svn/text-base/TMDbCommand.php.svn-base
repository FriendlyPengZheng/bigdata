<?php
/**
 * This file contains the TMDbCommand class.
 */

/**
 * TMDbCommand represents an SQL statement to execute against a database.
 *
 * It is usually created by calling {@link TMDbConnection::createCommand}.
 ...
 * can be accessed via the returned {@link TMDbDataReader}.
 *
 * TMDbCommand supports SQL statement preparation and parameter binding.
 ...
 *
 * TMDbCommand can also be used as a query builder
 ...
 * @property TMDbConnection $connection The connection associated with this command.
 ...
 */
class TMDbCommand extends TMComponent
{
    /**
    ...
    /**
     * @var TMDbConnection The connection associated with this command.
     */
    private $_connection;
    ...
    /**
     * Constructor.
     * @param TMDbConnection $connection the database connection
     ...
     * for more details.
     */
    public function __construct(TMDbConnection $connection, $query = null)
    {
     ...
     * @return TMDbCommand this command instance
     */
    ...
    /**
     * @return TMDbConnection the connection associated with this command
     */
     ...
     * automatically.
     * @throws TMDbException if TMDbCommand failed to prepare the SQL statement
     */
    public function prepare()
    {
        if ($this->_statement == null) {
            try {
                ...
                throw TMDbException::driverExceptionDuringPrepare($e, $this->getText());
            }
        }
    }
     ...
     * @return TMDbCommand the current command being executed
     */
     ...
     * @throws TMDbException execution failed
     */
    public function execute($params = array())
    {
        try {
            ...
            $this->_afterExecute($params);
            return $count;
        } catch(Exception $e) {
            throw TMDbException::driverExceptionDuringQuery($e, $this->getText(),
     ...
     * @param {array} $params
     */
    private function _afterExecute($params)
    {
        if ($this->_connection->logger && count($this->_connection->logger) == 2) {
            list($className, $method) = $this->getConnection()->logger;
        } else {
            return;
        }
        if (!$this->_connection->modelClass) return;
        if (($pars = array_merge($this->_paramLog, $params)) !== array()) {
            $p = array();
            foreach ($pars as $name => $value) {
                $p[$name] = $name . '=' . var_export($value, true);
            }
            $par = '. Execute with ' . implode(',', $p);
        } else {
            $par = '';
        }
        $class = new $className();
        $class->$method($this->_connection->modelClass, $this, $par);
    }
     ...
     * Please also note that all values are treated as strings in this case, if you need them to be handled as
     * their real data types, you have to use {@link bindParam} or {@link bindValue} instead.
     * @return TMDbDataReader the reader object for fetching the query result
     * @throws TMDbException execution failed
     */
     ...
     * Please also note that all values are treated as strings in this case, if you need them to be handled as
     * their real data types, you have to use {@link bindParam} or {@link bindValue} instead.
     * @throws TMDbException if TMDbCommand failed to execute the SQL statement
    ...
    private function _queryInternal($method, $mode, $params = array())
    {
        ...
        try {
            ...
            if ($method === '') {
                $result = new TMDbDataReader($this);
            } elseif ($method === 'fetchAll') {
                /**
                 * PDOStatement::setFetchMode doesn't support PDO::FETCH_GROUP,
                 * so we pass the mode in directly.
                 */
                $mode = (array)$mode;
     ...
     * @throws TMDbException if "from" key is not present in given query parameter
    ...
    public function buildQuery($query)
    {
        ...
        if (!empty($query['from']))
            ...
            throw new TMDbException('The DB query must contain the "from" portion.');
     ...
     * @return TMDbCommand the command object itself
     */
     ...
     * @param string $table the table that new rows will be replaced into.
     * @param array $columns the column data (name=>value) to be replaced into the table.
     * @return integer number of rows affected by the execution.
     */
     ...
     * @throws TMDbException if unknown operator is used
    ...
    private function _processConditions($conditions)
    {
        ...
        throw new TMDbException(sprintf('Unknown operator "%s".', $operator));
    }
