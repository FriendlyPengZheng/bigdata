<?php
/**
 * TMDbDataReader class file
 */

/**
 * TMDbDataReader represents a forward-only stream of rows from a query result set.
 *
 * To read the current row of data, call {@link read}. The method {@link readAll}
 * returns all the rows in a single array.
 *
 * One can also retrieve the rows of data in TMDbDataReader by using foreach:
 * <pre>
 * foreach($reader as $row)
 *     // $row represents a row of data
 * </pre>
 * Since TMDbDataReader is a forward-only stream, you can only traverse it once.
 *
 ...
 */
class TMDbDataReader extends TMComponent implements Iterator, Countable
{
    /**
     * @var PDOStatement The PDOStatement instance.
     */
    private $_statement;

    /**
     * @var boolean Whether the reader id closed or not.
     */
    ...
    /**
     * @var integer The index of the current row.
     */
    private $_index = -1;

    /**
     * Constructor.
     * @param TMDbCommand $command the command generating the query result
     */
    public function __construct(TMDbCommand $command)
    {
     ...
     * @throws TMDbException if this method is invoked twice
     */
    public function rewind()
    {
        if ($this->_index < 0) {
            ...
            throw new TMDbException('TMDbDataReader cannot rewind. It is a forward-only reader.');
    }
