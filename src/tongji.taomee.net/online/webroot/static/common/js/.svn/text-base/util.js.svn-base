    ...
    hint: function(time) {
    	...
    	for (var i = 0; i < _time; i++) {
    		window.setTimeout(function() {
    			obj.css('background-color', '#FFF2B1');
    		}, (i * 2 + 1) * 300);
    		window.setTimeout(function() {
...
download: function(url, param) {
    ...
    ajax(url[0], $.extend({}, parseArgs(url[1]), param), function(res) {
        if (res.result == 0) {
            ...
            if (typeof res.data.exists !== "undefined") {
                msg = "文件正在处理，请耐心等待，您也可以到页面右上角【我的下载】查看！"
                + "<a href='" + getUrl("tool/export/index")
                + "' title='现在就去' target='_blank' class='a-go'>现在就去</a>";
            } else if (typeof res.data.cost_time !== "undefined") {
                msg = "由于您选择的时间间隔较长，已自动为您异步下载，预计时间"
                + res.data.cost_time + "s，您可以到页面右上角【我的下载】查看和下载！"
                + "<a href='" + getUrl("tool/export/index")
                + "' title='现在就去' target='_blank' class='a-go'>现在就去</a>";
            } else if (typeof res.data.message !== "undefined") {
                msg = "文件下载出现错误，" + res.data.message
                + "<a href='" + getUrl("tool/export/index")
                + "' title='去查看' target='_blank' class='a-go'>去查看</a>";
            }
        } else {
