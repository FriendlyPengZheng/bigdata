...
$.datatable.sort = {
    ...
    "percentage-asc": function(x, y) {
        return x - y;
    },
    "percentage-desc": function(x, y) {
        return y - x;
    },
...
_create: function() {
    ...
    this.sort = [o.sortDefaultColumn, o.sortDefaultType, 1];
...
_initSearch: function() {
    var wrap = $("<div>").addClass("datatable-search"),
        label = $("<label>"),
        search = $("<input/>").addClass("search-ipt"),
        that = this, timeout,
        length, value, i;
...
_prepareSort: function() {
    ...
    for (i = 0; i < columnLength; i ++) {
        if (!tableColumns[i].disabled)  {
            $(tableColumns[i].th).addClass("sorting").append($("<i>"));
        } else if (i === this.sort[0]) {
            this.sort[0] ++; 
        }
