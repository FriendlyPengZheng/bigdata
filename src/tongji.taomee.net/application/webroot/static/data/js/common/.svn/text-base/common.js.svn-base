        ...
        $(data).each(function(i) {
            if(!this.ignore && !inArray(this.ignoreId, window.responseData.currentPage.ignore)) {
                if (window.responseData.isAdmin) {
                    this.ignoreTools = this.ignoreTools || "wrapIgnore";
                } else {
                    delete this.ignoreTools;
                }
...
window.wrapDownload = function(d) {
    var param = getPageParam("download"),
        toolsOption = d.getToolsOption ? d.getToolsOption() : {};
 ...
 * @return
 */
window.wrapIgnore = function(d) {
    var ignoreId = d.options.ignoreId,
        params = getPageParam();

    overlayer({ text: lang.t("操作中...") });
    ajax("index.php?r=admin/manage/ignore", {
        gameId: params.game_id,
        ignoreId: ignoreId
    }, function (res) {
        if (res.result == 0) {
            hidelayer(lang.t("操作成功~.~"));
        } else {
            hidelayer();
