<?php
class Navigator extends TMComponent
{
    /**
    ...
    public function getNavigator()
    {
        ...
        list($aNavigator, $aUrls, $deepLevel) = $this->getCompleteNavigatorInfo();
        // no authority
        if (empty($aNavigator)) {
        ...
        $topNaviId = $aCurrentMap[0];

        if (isset($deepLevel[$topNaviId])) {
            if ($deepLevel[$topNaviId] <= 3) {
                $aAside = $aSecondBar;                                                       
                $aSecondBar = array();                                            
            }
        }      
        $page = [];
        $continue = false;
        foreach ($aCurrentMap as $index => $naviId) {
            if (!$continue) {
                $page[$index] = $aNavigator[$naviId];
                unset($page[$index]['children']);
            }
            if (isset($aNavigator[$naviId]['game_related']) && $aNavigator[$naviId]['game_related']) {
                $continue = true;
            } else {
                $continue = false;
            }
        ...
        $pageTitle = implode('-', TMArrayHelper::column($page, 'name'));                                                                                  
        $pageKey   = implode('.', array_filter(TMArrayHelper::column($page, 'key')));
    ...
    protected function getGameNavigator($sGameFunction)
    {
        ...
        foreach ($aAuthGame as $gameId => &$gameInfo) {
            ...
            if (!isset($aUrls[$sGameTypeRoute])) {
                $gameInfo['url'] = strpos($this->_aGameTypes[$gameInfo['game_type']], '?') === false ? ($this->_aGameTypes[$gameInfo['game_type']].
                    '?game_id=' . $gameInfo['game_id']) : ($this->_aGameTypes[$gameInfo['game_type']] . '&game_id=' . $gameInfo['game_id']);
            } else {
    ...
    protected function parseRawNavigator(&$aRawNavigator, &$aAuthIds)
    {
        ...
        $deepLevel = $this->stripUrl($aNavigatorInfo, $aNavigatorUrls);
        return array($aNavigatorInfo[1][0], $aNavigatorUrls, $deepLevel);
    }
                        ...
                        if (isset($aUrls[$iNaviLevel]) && isset($aUrls[$iNaviLevel][$naviId])) {
                            foreach ($aUrls[$iNaviLevel][$naviId] as $value) {
                                ...
                                if (!$iNextLevel) {
                                    ...
                                    if (isset($deepLevel[$naviId])) {
                                        $deepLevel[$naviId] = $deepLevel[$naviId] > count($value['ids']) ? $deepLevel[$naviId] : count($value['ids']);
                                    } else {
                                        $deepLevel[$naviId] = count($value['ids']);
                                    }
                                }
                            }
                        }
