...
class data_Data extends TMModel
{
    ...
    /**
     * @var array data_configuration_Data
     */
    protected $dataConfigurations;
    ...
    /**
     * @brief getDataConfigurations 
     * 获取数据配置，可以从中获取分组情况，用于数据元的分组下载等
     *
     * @return {array}
     */
    public function getDataConfigurations()
    {
     ...
     * @param $authorize
     */
    protected function addUserParamToConfiguration($userParam, $byItem, $authorize)
    {
        ...
        $this->dataConfigurations[] = $dataConfModel;
        $this->addConfiguration($dataConfModel, $gpzsConf, $authorize);
    }
     ...
     * @param {boolean} $authorize gpzs是否需要权限控制，只能用于内部获取特殊数据设置
     */
    public function getTimeSeries($aUserParam, $authorize=true)
    {
        ...
        foreach ($aUserParam['data_info'] as $dataIdx => $dataParam) {
            ...
            if (($userParam = $this->addUserParam($dataParam))) {
                $this->addUserParamToConfiguration($userParam, $aUserParam['by_item'], $authorize);
            }
        }
     ...
     */
    protected function addConfiguration($dataConfiguration, $gpzsConfigure, $authorize=true)
    {
            ...
            foreach ($dataConfiguration->getDataConfigure() as $key => $item) {
                if (isset($dataList[$key])) {
                    foreach ($dataList[$key] as $index => $dataInfo) {
                        ...
                        $dataName = isset($item['data_name']) ? 
                            (is_array($item['data_name']) && isset($item['data_name'][$index]) ?
                                $item['data_name'][$index] : 
                                $item['data_name']) :
                            $dataInfo['data_name'];
                        $dataInfoKey = array_map(function($n) { return '{' . $n . '}'; }, array_keys($dataInfo));
                        $configuration->setDataName(TM::t('tongji', $dataName, array_combine($dataInfoKey, $dataInfo)))
                        ...
                        if (!$authorize) $configuration->closeAuthorize();
